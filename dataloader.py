...
def buildLabelIndex(labels):
    ...
    for idx, label in enumerate(labels):
        if label.item() not in label2inds:
            label2inds[label.item()] = []
        label2inds[label.item()].append(idx)
...
class GenericDataset(data.Dataset):
    ...
    def _keep_first_k_examples_per_category(self, num_imgs_per_cat):
        if self.dataset_name == 'mnist':
            ...
            raise ValueError(
                'Not recognized dataset {0}'.format(self.dataset_name))
        ...
        if self.unsupervised:
            ...
            def _load_function(idx):
                ...
                rotated_imgs = [
                    self.transform(img0),
                    self.transform(
                        np.flip(rotate_img(img0,  90), axis=0).copy()),
                    self.transform(
                        np.flip(rotate_img(img0,  180), axis=0).copy()),
                    self.transform(
                        np.flip(rotate_img(img0,  270), axis=0).copy())
                ]
...
if __name__ == '__main__':
    ...
    dataset = GenericDataset('mnist', 'train')
    ...
    for i in range(data.size(0)):
        ax = plt.subplot(int(data.size(0)/4), 4, i+1)
